...
class ImprovedMemorySystem:
    ...
    # ================================================================================
    # CONTEXT INJECTION (for bluetools.py compatibility)
    # ================================================================================

    def should_inject_context(self, messages: list) -> bool:
        """
        Determine if historical context should be injected.
        Returns True if we have facts that might be relevant.
        """
        # Always inject if we have facts
        return len(self._facts_cache) > 0

    def build_context(self, messages: list, user_name: str = None) -> list:
        """
        Build context messages to inject into the conversation.
        Returns a list of message dicts with relevant facts.
        """
        if not self._facts_cache:
            return []

        # Build a context message with key facts
        ...
        priority_keys = ['user_name', 'name', 'location', 'occupation', 'partner_name',
                        'children_names', 'workplace', 'age', 'birthday']

        # Add priority facts first
        for key in priority_keys:
            if key in facts:
                fact_lines.append(f"- {key}: {facts[key]}")
        ...
        context_content = "Here are facts I remember about the user:\n" + "\n".join(fact_lines)
    ...
    def consolidate_if_needed(self, user_name: str = None) -> bool:
        """
        Run consolidation if needed (e.g., too many low-confidence facts).
        Called periodically by bluetools.py after conversations.
        """
        stats = self.get_stats()

        # Consolidate if we have pending conflicts or many facts
        should_consolidate = (
            stats['pending_conflicts'] > 5 or
            stats['total_facts'] > 100
        )

        if should_consolidate:
            self.consolidate_facts()
            return True
    ...
    def get_memory_summary(self) -> dict:
        """
        Get a comprehensive memory summary for the /memory endpoint.
        Returns a dict with facts, stats, and metadata.
        """
        stats = self.get_stats()
        facts = self.get_all_facts()
        ...
        return {
            ...
            'facts': facts,
            'conflicts': conflicts,
            'max_version': stats.get('max_version', 1),
            'avg_version': stats.get('avg_version', 1.0)
        }
